...
import { getMove, typeMultiplier } from "./moves.js";
...
export function resolveAttack(attacker, defender, moveId) {
  ...
  const log = { moveName: move.name, hit: false, fainted: false, damage: 0, effectApplied: false };
  ...
  if (move.effect && Math.random() < move.effect.chance) {
    if (move.effect.stat === "spd") {
      defender.spdStage = Math.max(-2, Math.min(2, defender.spdStage + move.effect.stages));
...
export function grantExp(entry, amount) {
  ...
  let leveledUp = false;
  while (updated.exp >= EXP_PER_LEVEL * updated.level && updated.level < 60) {
